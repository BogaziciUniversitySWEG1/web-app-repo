...
(function () {
    var SP_BANK = {
        ...
        GetUpcomingEvents: function (communityId, callback, callback_err) {
            try {
                $.ajax({
                    type: "GET",
                    url: "api/meetings/" + communityId+"?status=0", 
                    contentType: "application/json; charset=utf-8",
                    success: function (msg) {
                      	if (msg == null) {
                            callback(null);
                            ...
                            callback_err(msg.status, 'Get upcoming events');
                        }
                        else {
                            var _data = eval(msg);
                            callback(_data);
                    ...
                    error: function (msg) {
                        callback_err(msg.status, 'Get upcoming events Fails. Reason: ' + (msg.statusText));
                    }
                });
            }
            catch (err) {
                 callback_err(-2, 'Get upcoming events Fails. Reason: ' + err.Description);
            }
        },
                ...
                $.ajax({
                    ...
                    success: function (msg) {
                      	if (msg == null) {
                            ...
                            callback_err(msg.status, 'Get community Meetings');
                        }
                        else {
                            var _data = eval(msg);
                            callback(_data);
                    ...
                    error: function (msg) {
                        callback_err(msg.status, 'Get communityMeetings Fails. Reason: ' + (msg.statusText));
                    }
                });
            }
            catch (err) {
